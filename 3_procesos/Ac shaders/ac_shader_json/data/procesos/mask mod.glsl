            ...
            vec3 p=permute(
                ...
                void main(){
                    ...
                    */
                    ...
                    pct=distance(st,vec2(.710,.100))/.856;
                    
                    // b. The LENGTH of the vector
                    //    from the pixel to the center
                    vec2 toCenter=vec2(.410,.630)-st;
                    pct*=length(toCenter)*5.036;
                    toCenter=vec2(.040,.670)-st;
                    pct*=length(toCenter);
                    toCenter=vec2(.60,.870*abs(sin(time*.25)))-st;
                    pct*=length(toCenter)*2.884;
                    toCenter=vec2(.850,.600)-st;
                    pct*=length(toCenter*2.036);
                    // c. The SQUARE ROOT of the vector
                    //    from the pixel to the center
                    vec2 tC=vec2(.200,.240)-st;
