...
class Ac {
  ...
  String acSimple="ac array.glsl";//"ac array - res.glsl";
  ...
  void dibujar(Movie movi) {
    this.dibujar(movi, posX, posY, w, h);
  }
  void dibujar(Movie movi, float x_, float y_) {
    //dibujar(img_, x_,  y_, 100, 100 ); 
    this.dibujar(movi, x_, y_, w, h);
  }


  void dibujar(Movie movi, float x_, float y_, float w_, float h_) {
    //background(170);
    ...
    //salida = createImage(w, h, RGB);
    salida.copy(movi, 0, 0, movi.width, movi.height, 0, 0, int(pg.width*zoomImgX), int(pg.height*zoomImgY));
    // (img_.width+pg.width)/2, (img_.height+pg.height)/2,
    salida.mask(pg);
