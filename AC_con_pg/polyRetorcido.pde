...
class polyRetorcido {
  ...
  polyRetorcido(int w_, int h_, float radio_, float init_, int lados_, int interpolaciones) {
    ...
    interA=interArray(cX, interpolaciones);
    interB=interArray(cY, interpolaciones);
   // println("antecesor de interX: ",interA.length);
    reArray() ;
  }
  
  
  void reArray() {
   // println("reArray funciona bien?");
    //funcion para el draw - copia el array para modificar 
    interX=interA;//    interX=interA.clone();
    interY=interB;//    interY=interB.clone();
    //println ("interX  ",interX.length);
    //println ("interY  ", interY.length);
  }
  //  void newSet(float x_, float y_, float radio_, float init_, int lados_, int interpolaciones) {

  void newSet(float radio_, float init_, int lados_, int interpolaciones) {
    cX=polyCos(base.width/2, radio_, init_, lados_);
    cY=polySin(base.height/2, radio_, init_, lados_);
    interA=interArray(cX, interpolaciones);
    interB=interArray(cY, interpolaciones);
