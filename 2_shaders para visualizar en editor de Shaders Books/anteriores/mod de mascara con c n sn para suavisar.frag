...
void main(){
   ...
   // e += noise(uv2)*0.5;
    ...
    // e e = snoise(uv2);-= voronoi(uv2);
    
       //e = fbm(uv2);
        //e = fbm(uv*5.91+fbm(uv*009.905+fbm(uv2-time*0.5)+time*0.001));
        //e += fbm(vec2(e)*uv2);
      //e = ridgedMF(vec2(ridgedMF(uv2)-time*0.2));
   ...
   /// e= pct;
    ...
    st -=1.;
     ...
     pct = 01.0;
      toCenter = vec2(-0.580,0.330)-st;
   	pct = length(toCenter)*1.836;
    
     color = vec3(pct);
     vec2 st2 =st +vec2(0.580,0.180);
    color *= vec3(circle(st2,1.060));
