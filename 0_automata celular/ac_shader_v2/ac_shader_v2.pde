...
String path = "F:/Documentos/programacion/processing/sketchs/mae/proyectos mae/tiempo/carpetas/ernest";
...
void setup() {
  ...
  for (int i=0; i< acList.size(); i++) {
    ...
    rFuturoMin[i] = int(random(10, 40));
    rFuturoMax[i] = int(random(40, 70));
...
void draw() {
  ...
  for (int i=0; i< acList.size(); i++) { 
    ...
    acTemp.dibujar(listImg[(contador[i]+acTemp.id)%listImg.length], locTempX.get(i), locTempY.get(i), tamTempX.get(i), tamTempY.get(i));
