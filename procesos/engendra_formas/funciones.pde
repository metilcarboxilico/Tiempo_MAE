...
//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//---//
/*
__tablas de seno y coseno
 __interpolaciones
 __poligonos  (regulares en forma: vec, y en dos funciones de array de x e y)
 __modificadores de arrays floats
...
//no implemetadas a este nivel procesamiento
...
float[] interArray(float[] a, int cant) {
  ...
  for (int i = 0; i < a.length; i++) {
    ...
    if (a.length-1==i)  temp= interpolar(a[i], a[0], cant);
    else temp= interpolar(a[i], a[i+1], cant) ; 
...
/*
float[] interpolar(float a, float b, int cant) {
 //funcion que suma los extremos y que  dejo provisoriamente
 int div=cant+2;
...
float[] senoArray(float[] array_, float amp, float frec, float offset) {
  ...
  for (int i=0; i<array.length; i++) {
    ...
    if (frec==0) frec=1;
    array[i]=array[i]+sin(i*(TAU/frec)+offset)*amp;
  }
  return array;
}
...
float[] cosenoArray(float[] array_, float amp, float frec, float offset) {
  ...
  for (int i=0; i<array.length; i++) {
      if (frec==0) frec=1;
    array[i]=array[i]+cos(i*(TAU/frec)+offset)*amp;
  }
  return array;
}
